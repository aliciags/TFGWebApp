...
export const getMenuDays = async ( req: Request, res: Response ) => {
    ...
    try {
        ...
        if(!days){
            return res.status(HttpStatusCodes.BAD_REQUEST).json({msg: "There is no such menu"});
        }
        res.json(days);
    } catch(err){
        console.error(err.message);
...
export const getDay = async ( req: Request, res: Response ) => {
    ...
    try {
        ...
        if(!day){
            return res.status(HttpStatusCodes.BAD_REQUEST).json({msg: "Day does not exist"});
        }
        res.json(day);
    } catch(err){
        console.error(err.message);
        if(err.kind === "ObjectId"){
            return res.status(HttpStatusCodes.BAD_REQUEST).json({msg: "not a day object id"});
        }
        res.status(HttpStatusCodes.INTERNAL_SERVER_ERROR).json({msg: err.message});
    }
}
...
export const addDay = async ( req: Request, res: Response ) => {
    ...
    try {
        let day: IDay = new Day(dayFields);
        await day.save();
        res.json(day);
    } catch(err){
        console.error(err.message);
